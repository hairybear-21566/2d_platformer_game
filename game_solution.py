...
class GameStateManager:
    ...
    def resume_game(self) -> None:
        ...
        """
        Deletes all buttons on the screen and resumes the game loop.

        This method is called when the player chooses to resume the game from the
        pause menu. It destroys the buttons on the screen and calls the
        `playing_in_game_menu_button` method to recreate the in-game pause menu
        button. Then, it calls the `game_loop` method to resume the game loop.
    ...
    def cheat_code_entry(self) -> None:
        ...
        def go_back() -> None:
            """
            Destroys all buttons on the screen and then goes back to the in game pause menu

            This method is called when the player chooses to go back to the in game pause menu
            from the cheat code page. It destroys all buttons on the screen and then calls the
            `in_game_pause_menu` method to recreate the in-game pause menu buttons.
            """
    ...
    def key_press(self, event) -> None:
        """
        Handles key press events for various game actions.

        This method listens for key presses and triggers corresponding actions such as
        player jump, boss key activation, and cheat code toggles.

        Args:
            event: The event object containing information about the key press.

        Key Bindings:
            - Jump: If the pressed key is the current jump key, the player jumps.
            - Boss Key: If 'b' is pressed and the entry field is not focused, the boss key window is created.
            - Triple Points: If 't' is pressed and the entry field is not focused, the triple points cheat is toggled.
            - Super Jump: If 'o' and 'p' are pressed (while entry field is not focused), super jump is activated.
        """
    ...
    def key_release(self, event) -> None:
        ...
        - Jump: If the released key is the current jump key, the player stops jumping.
        - Super Jump: If 'o' or 'p' is released and the entry field is not focused, super jump is deactivated.
        """
    ...
    def game_loop(self) -> None:
        ...
        This function is called every 20 milliseconds and checks for various
        conditions, such as whether the player is on the ground, whether the
        player has collided with a bird, and whether the player has reached the
        score limit. If any of these conditions are met, the game loop is stopped
        and the game is restarted. Otherwise, the player, backgrounds, and
        platforms are updated and the game loop is continued.

        The function is initially called in the __init__ method and is
        repeatedly called until the game is stopped.
        """
    ...
    def restart_game(self) -> None:
        ...
        """
        Count down display routine
        This function is called when a game is completed, it displays a countdown on the screen
        from 3 to 1, then resets the game by calling the game loop function.
        The function also resets the platforms, player and score, and sets the birds starting positions.
        """
        def count_down_display(n: int) -> None:
            """
            Displays a countdown image on the screen from 3 to 1.

            Args:
            n (int): The number to display in the countdown.
        ...
        def delete_countdown_label(id: int) -> None:
            """
            Deletes a countdown image from the canvas.
