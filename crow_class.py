...
class Crow:
    ...
    def update_crow(self, player, restart_game) -> bool:
        # crow translated
        """
        Updates the crow's position and animation, checks for collision with the player,
        and potentially restarts the game if a collision is detected.

        Args:
            player (list[int]): The hitbox of the player for collision detection.
            restart_game (function): Function to call to restart the game upon collision.

        Returns:
            bool: Returns False if a collision with the player occurs, triggering a game restart.
    ...
    def animate(self, f_path: str) -> None:
        """
        Updates the crow's image to the given file path.

        Args:
            f_path (str): The file path of the new image.
        """
    ...
    def move(self) -> None:
        ...
        """
        Moves the crow sprite across the screen. If the crow moves off the screen,
        it repositions the crow to a random height at the right end and adjusts the scroll speed.
        Updates the crow's position and hitbox on the canvas.
        """
    ...
    def get_min_height(self) -> int:
        """
        Returns the minimum height of a bird, which is the minimum y value it can occupy.

        Returns:
            int: The minimum height of a bird.
        """
