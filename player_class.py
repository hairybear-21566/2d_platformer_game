...
class Player:
    ...
    def animate(self):
        ...
        If the player is not on the ground, it will display the jump animations.
        If the player is on the ground, it will display the landing animations if the player is still landing.
        Otherwise, it will display the running animations.
        """
    ...
    def reset_frames(self):
        """
        Resets the player's animation frames and state to their initial values.

        This method sets the running, jumping, and landing frames to their starting
        positions, updates the player's image to the initial jumping sprite, and
        resets flags related to landing and jump animation states.
        """
        self.run_frame = 0  # frames range from 0 to 7
        self.jump_frame = 2  # frames range from 0 to 2
        self.land_frame = 0  # frames range from 0 to 8
    ...
    def move_player(self, dx: int, dy: int):
        """
        Move the player by the given amount of pixels.

        Args:
            dx (int): The number of pixels to move the player horizontally.
            dy (int): The number of pixels to move the player vertically.

        The player is moved by updating the player's position and repositioning the image on the canvas.
        """
    ...
    def set_new_floor(self, platforms: list):
        """
        Checks if there is a platform directly under the player.
        If there is, set the player's floor to the height of the platform.
        If there is not, set the floor to a large number, effectively allowing the player to fall to their death.
    ...
    def set_on_ground(self):
        ...
        Sets the player's on_ground status to True if the player's bottom
        (y + size) is at the same height as the floor, and False otherwise.
        """
    ...
    def set_available_jump(self):
        ...
        Sets the player's jump availability to True if the player's bottom
        (y + size) is at the same height as the floor, and False otherwise.
        """
        if self.y+self.size == self.floor:
    ...
    def reset_position(self):
        """
        Reset the player's position to the default starting point and reset their animation
    ...
    def reset_image(self):
        """
        Resets the player's image to the initial position on the canvas.

        After a player's image is moved or resized, this method is called to
        move the image back to its original position on the canvas. This is
        useful for resetting the player's position after a game over or for
        updating the player's position on the canvas after resizing the window.
        """
